  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 flex flex-col items-center">
      <div className="text-center space-y-2">
        ...
        <p className="text-lg text-slate-500">Voltea la tarjeta para descubrir la respuesta.</p>
      </div>
      ...
      <div className="w-full relative h-96 perspective-1000">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`absolute inset-0 w-full h-full glass-card p-10 flex flex-col justify-center items-center text-center cursor-pointer shadow-2xl ${isFlipped ? 'bg-primary-50/90' : 'bg-white/90'}`}
            onClick={() => setIsFlipped(!isFlipped)}
            style={{ transformStyle: 'preserve-3d' }}
          >
            <div className="absolute top-6 left-6 px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-sm font-medium">
              {currentCard.subject}
            </div>
            <div className="absolute top-6 right-6 text-slate-400 font-medium">
            ...
            <p className="absolute bottom-6 text-sm text-slate-400 font-medium">
      ...
      <div className="flex gap-4">
        <button onClick={prevCard} className="p-4 rounded-full bg-slate-200 text-slate-600 hover:bg-slate-300 transition-colors">
          <ArrowLeft size={24} />
        </button>
